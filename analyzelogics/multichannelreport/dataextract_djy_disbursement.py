...
reporttype='dataextract_djy_disbursement'
...
def deletebatch(batchid):
    try:
        conn = pymysql.connect(host=st.secrets["mysql"]['host'],
                               ...
                               db=st.secrets["mysql"]['database'])
        cursor = conn.cursor()
        sql0 = f"""delete from  newchannel_djy_disbursement where batchid = '{batchid}' """
        cursor.execute(sql0)
...
def dealsinglefile(path, attrjson):
    try:
        ...
        df.to_sql('newchannel_djy_disbursement', con=engine, if_exists='append', index=False, index_label=False)
