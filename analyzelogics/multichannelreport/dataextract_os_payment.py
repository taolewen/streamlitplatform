...
reporttype='dataextract_os_payment'
...
def deletebatch(batchid):
    try:
        conn = pymysql.connect(host=st.secrets["mysql"]['host'],
                               ...
                               db=st.secrets["mysql"]['database'])
        cursor = conn.cursor()
        sql0 = f"""delete from  newchannel_os_payment where batchid = '{batchid}' """
        cursor.execute(sql0)
...
def dealsinglefile(path, attrjson):
    try:
        ...
        df = pd.read_excel(path,sheet_name='Detail',names=name)
        ...
        df.to_sql('newchannel_os_payment', con=engine, if_exists='append', index=False, index_label=False)
