    ...
    try:
        ...
        if len(str(attrjson['week']))==8:
            ...
            if attrjson['country']=='US':
                ...
                name=['订单类型','订单号','站点','换货订单','订单状态','发货模式','是否催发','是否丢件','是否滞留','问题订单','商品名称',
                      '货号','规格','卖家SKU','SHEIN_SKU','SKC','商品ID','商品状态','库存标识','换货标识','换货原因','被换商品ID','是否锁定','分单时间','要求发货时间',
                      '运单号','卖家包裹','卖家币种','商品价格','MX个人卖家汇总税费','MX税费子科目_WHT_VAT','MX税费子科目_WHT_IT','MX税费子科目_Cedular_WHT','消费税','预计收入']
                df = pd.read_excel(path, usecols='A:AI', names=name)
                df['要求签收时间']=None
            else:
                name=['订单类型','订单号','站点','换货订单','订单状态','发货模式','是否催发','是否丢件','是否滞留','问题订单','商品名称',
                      '货号','规格','卖家SKU','SHEIN_SKU','SKC','商品ID','商品状态','库存标识','换货标识','换货原因','被换商品ID','是否锁定','分单时间',
        ...
        # ]]
        df=df[['area','country','store','week','qijian',
               '订单类型', '订单号','站点', '换货订单', '订单状态', '发货模式', '是否催发', '是否丢件', '是否滞留', '问题订单', '商品名称',
