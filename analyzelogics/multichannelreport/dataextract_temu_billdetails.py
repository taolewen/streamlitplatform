...
reporttype='dataextract_temu_billdetails'
...
def deletebatch(batchid):
    try:
        conn = pymysql.connect(host=st.secrets["mysql"]['host'],
                               ...
                               db=st.secrets["mysql"]['database'])
        cursor = conn.cursor()
        sql0 = f"""delete from  newchannel_temu_billdetails where batchid = '{batchid}' """
        cursor.execute(sql0)
...
def dealsinglefile(path,attrjson):
    try:
        ...
        # ]]
        df=df[['area','country','store','week','qijian',
               '账务时间','账务类型','币种','收支金额_USD','备注',
               "batchid"
        ]]

        df.to_sql('newchannel_temu_billdetails', con=engine, if_exists='append', index=False, index_label=False)
