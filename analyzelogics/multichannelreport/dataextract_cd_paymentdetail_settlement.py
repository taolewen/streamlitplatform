...
reporttype='dataextract_cd_paymentdetail_settlement'
...
def deletebatch(batchid):
    try:
        conn = pymysql.connect(host=st.secrets["mysql"]['host'],
                               ...
                               db=st.secrets["mysql"]['database'])
        cursor = conn.cursor()
        sql0 = f"""delete from  newchannel_cd_paymentdetail_settlement where batchid = '{batchid}' """
        cursor.execute(sql0)
...
def dealsinglefile(path,attrjson):
    try:
        ...
        df.to_sql(name='newchannel_cd_paymentdetail_settlement', con=engine, if_exists='append', index=False, index_label=False)
