...
reporttype='dataextract_mm_order'
...
def deletebatch(batchid):
    try:
        conn = pymysql.connect(host='124.71.174.53',
                               ...
                               database='csbd')
        cursor = conn.cursor()
        sql0 = f"""delete from  newchannel_mm_order where batchid = '{batchid}' """
        cursor.execute(sql0)
...
def dealsinglefile(path,attrjson):
    try:
        ...
        df.to_sql('newchannel_mm_order', con=engine, if_exists='append', index=False, index_label=False)
