...
reporttype='dataextract_mm_storagefeesmf'
...
def deletebatch(batchid):
    try:
        conn = pymysql.connect(host=st.secrets["mysql"]['host'],
                               ...
                               db=st.secrets["mysql"]['database'])
        cursor = conn.cursor()
        sql0 = f"""delete from  newchannel_mm_storagefeesmf where batchid = '{batchid}' """
        cursor.execute(sql0)
...
def dealsinglefile(path,attrjson):
    try:
        ...
        df.to_sql('newchannel_mm_storagefeesmf', con=engine, if_exists='append', index=False, index_label=False)
